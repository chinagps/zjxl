 ...
 * @param [,] defCenter 默认的中心位置经纬度数组
 * @param {Int} defLvl 默认地图级别(可选,如不传,则为5 )
 * @return {Object} CTFO_MAP对象的引用
 */
var CTFO_MAP = function(mCtnr, defCenter, defLvl) {
    var that = this;
    if (!defCenter){
    	defCenter = [116.40969,39.94940];//北京
    }
    if (!defLvl) 
        defLvl = 4;
    this.mtool = null;
	this.map = new TMMaps(mCtnr);
	this.map.centerAndZoom(new TMLngLat(defCenter[0],defCenter[1]),defLvl);
	this.map.handleMouseScroll(true);
	this.map.enableDoubleClickZoom();
	this.map.closeHighlight();//关闭高亮
...
//  this.map.removeControl(this.map.logoControl);
     ...
     * 添加地图版权信息
     */
    this.addMapCopyRight = function(p){
...
//    this.map.addControl(cr);
    ...
    this.addPointMarker = function(p){
    	...
    	option.lnglat=new TMLngLat(p.lng, p.lat);
    	var _pointMarker = new TMMarkerOverlay(option);
    	_pointMarker.setIcon(new TMIcon( p.iconUrl,new TMSize(20,20)));//'<img src="' + p.iconUrl + '"/>');
    	_pointMarker.setZindex(10000);
		this.map.overlayManager.addOverLay(_pointMarker);
    ...
    this.movePointMarker = function(p){
    	if(this.pointMarkerObj && this.pointMarkerObj[p.id]){
	    	...
	    	if (p.iconUrl) 
	        {
	    		_pointMarker.setIcon(new TMIcon( p.iconUrl,new TMSize(20,20)));
...
//            _icon.removeShadow();
        ...
        if (!this.markerObj[params.id]) {   
			...
			//设置点标注属性   
			var markerOptions = new TMMarkerOptions();   
			markerOptions.lnglat =_lngLat;
			markerOptions.icon=_icon;
			_marker= new TMMarkerOverlay( markerOptions );
        ...
        this.map.overlayManager.addOverLay(_marker);
        
        //	Marker's Label  在地图上指定地理位置的锚点处显示文字
        if (params.label) { 
        	if(!params.labelAnchor)
        		params.labelAnchor = [0.7,12];
        	var option = new TMHtmlOptions();    
        	option.editable=false;   
        	option.lnglat = _marker.getLngLat();  
        	option.offset=new TMPoint(params.labelAnchor[0],params.labelAnchor[1]); 
        	var title = params.title ? params.title : params.label;
        	option.content=title;
        	 if(params.labelFontColor){
        	 	option.fontColor=params.labelFontColor;
        	 }
        	params.labelBgColor=params.labelBgColor?params.labelBgColor:"#545454";
        	params.labelFontColor=params.labelFontColor?params.labelFontColor:"#FFFFFF";
        	params.labelFontSize=params.labelFontSize?params.labelFontSize:"10";
        	
        	option.content="<span style=\"border: 1px solid #ADAEAC"
        			+"; font-size: "+params.labelFontSize+"px; background-color: "+params.labelBgColor
        			+"; color: "+params.labelFontColor+"; padding: 3px;\" "
        			+"title=\""+title+"\">"+params.label+"</span>";
            var pOvrLy = new TMHtmlOverlay(option);//[0.7,-0.5]
            this.markerLblObj[params.id] = pOvrLy;
            this.map.overlayManager.addOverLay(pOvrLy);
        }
        //	tip marker弹窗
        if (params.isDefaultTip && params.tip) {//默认风格
        	...
        	option.editable=false;   
        	option.lnglat = _marker.getLngLat();
        	option.offset=new TMPoint(0.52,1.02); 
        	option.content=params.tip;
        	_selfDefineTip = new TMHtmlOverlay(option);//[0.58,1.16]
        }
        ...
        if(params.handler){//自定义marker点击事件
            ...
            if(params.isDefaultTip){//默认风格的点击事件,closeInfoWindow/openInfoWinHtml
            	TMEvent.addListener(_marker, "click", function(e){
            		...
            		_marker.openInfoWindow(params.title ? params.title : params.label,_selfDefineTip);
            	});
            	...
            	TMEvent.addListener(_marker, "click", function(e){
	            	...
	            	that.map.overlayManager.addOverLay(_selfDefineTip);
	  ...
	  if (_selfDefineTip && params.isOpen && !params.isDefaultTip){//自定义风格tip
        ...
        //貌似不起作用，注释掉，于2011.12.14 by FanXuean
//        if(params.isOpen){
    ...
    this.moveMarker = function(params) {
        if (this.markerObj && this.markerObj[params.id]) {
            ...
            if (this.markerLblObj && this.markerLblObj[params.id]) 
            {
                ...
                if(params.label){
//                	this.markerLblObj[params.id].setContent(params.label);
                	var title = params.title ? params.title : params.label;
                	var content="<span style=\"border: 1px solid #ADAEAC"
            			+"; font-size: "+params.labelFontSize+"px; background-color: "+params.labelBgColor
            			+"; color: "+params.labelFontColor+"; padding: 3px;\" "
            			+"title=\""+title+"\">"+params.label+"</span>";
                	this.markerLblObj[params.id].setContent(content);
                }
            }
...
//        var _pl = new TMPolyline(_arrSE_LngLat, params.strColor, params.numWdth, params.numOpacity);
        var lineOptions = new TMLineOptions();   
        lineOptions.lnglats = _arrSE_LngLat;   
        lineOptions.color=params.strColor;
        lineOptions.weight=params.numWdth;
        lineOptions.opacity=params.numOpacity;
        var _pl=new TMLineOverlay(lineOptions);
        this.polyLineObj[params.id] = _pl;
        this.map.overlayManager.addOverLay(_pl);
    ...
    this.addPolygon = function(params) {
        ...
        this.map.overlayManager.addOverLay(_pg);
        return _pg;
    };
    ...
    this.addEllipse = function(params) {
	    ...
	    if(!this.ellipseObj[params.id]){
			...
			this.map.overlayManager.addOverLay(_geoEllipse);
	    }else{
...
//    	this.map.overlayManager.addOverLay(marker);
//    };
    /**
     * 是否允许使用鼠标滚轮缩放地图
     *
