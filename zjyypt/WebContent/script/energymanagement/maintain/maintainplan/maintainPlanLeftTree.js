var maintainPlanLeftTree = function(){
...
maintainPlanLeftTree.prototype = {
		init : function(){
			this.load();
		},
		load : function(){
	 		$("#accordionplan1").ligerAccordion({changeHeightOnResize : true,speed : null});
			var obj = this;
			obj.setThree(obj);
		},
		setThree : function(o){
			var treeClick = "treeClick";
			$("#panelConplan3").ligerTree({
				url: 'operationmanagement/findOrgTree.action',
				onBeforeExpand : function(note){
					o.treeClick(note);
				},
				onSuccess : function(){
					o.selectNode("3",false);
					o.leafCoverFolder();
//					$("#searchForvehicleReportButton").trigger('click');
					o.bindPanelSize();
				},
				checkbox : true,
				childType : 'entType'
			});
			o.treeTreeManager = $("#panelConplan3").ligerGetTreeManager();
	 	},
	 	//获取树的高度，以便增加滚动条
	 	bindPanelSize : function(){
	 		var obj = this;
			var panelContextObj = $("#panelConplan3");
			//计算高度
//			var hgt = 260;
//			if (obj.getPosition(panelContextObj)){
////				hgt=$(window).height()-obj.getPosition(panelContextObj)-50;
//				hgt=KCPT.root.getCenterSize().height - 170;
//			}
			panelContextObj.css("overflow","auto").css("width","260px");
			panelContextObj.find("li").eq(0).width(400);
	 	},
	 	onResize:function(){
			$("#panelConplan3").height(getHeightAndWidth().height-205);
		},
		...
		treeClick : function(note) {
			try{
				var dix = maintainPlanLeftTree.getDisplayPanel();
				var manager = maintainPlanLeftTree.getManager(dix);
				var id = note.data.id;
				...
				if(entType == "2"){
					if(dix == "3"){
						if (note.data.childrenList=="" && nodeType == "2"){
							...
							maintainPlanLeftTree.ajaxTreeNode(url,note,id,manager,dix);
						}
					}else if(dix == "4"){
						if (note.data.childrenList=="" && nodeType == "2"){
							var tg = note.target;
		...
		ajaxTreeNode : function(url,note,id,manager,idx){
	    	$.ajax({
			   ...
			   success: function(col){
				   maintainPlanLeftTree.addTreeNode(note,col,manager);
				   maintainPlanLeftTree.selectNode(idx,false);
				   maintainPlanLeftTree.selectSubNodeAll(note.target);
			   }
			});
	    },
	    ...
	    selectNode : function (idx,flag)
	    {
	    	var a = null;
	    	if(idx == "3"){
	    		a = $("#panelConplan3 > li > div > div[class*=l-checkbox]");
	    	}
		...
		leafCoverFolder : function(){
	    	$("#panelConplan3").find("div[class*=l-note]").removeClass("l-note").addClass("l-expandable-close");
	    },
	    bindResultEvent : function(){
			var obj = this;
			$("#resultplanUp").click(function(){
				obj.treeSearchUp();
			});
			$("#resultplanDown").click(function(){
				obj.treeSearchDown();
			});
		},
		//查询结果上移一位
		treeSearchUp : function(idx){
			var obj = this;
			var resultArr = maintainPlanLeftTree.searchArr;
			var manager = obj.treeTreeManager;
			data = manager.getData();
			if(maintainPlanLeftTree.resultIdx > 0){
				maintainPlanLeftTree.resultIdx = Number(idx);
				obj._getDataNodeByTreeDataIndex(data[0].childrenList,resultArr.Rows[maintainPlanLeftTree.resultIdx]);
				obj.searchSel(resultArr.Rows[maintainPlanLeftTree.resultIdx].vid,resultArr.Rows[maintainPlanLeftTree.resultIdx].vehicleNo);
			}else{
		...
		treeSearchDown : function(idx){
			...
			var manager = obj.treeTreeManager;
			data = manager.getData();
			if(maintainPlanLeftTree.resultIdx < resultArr.Rows.length-1){
				maintainPlanLeftTree.resultIdx = Number(idx);
				obj._getDataNodeByTreeDataIndex(data[0].childrenList,resultArr.Rows[maintainPlanLeftTree.resultIdx]);
				obj.searchSel(resultArr.Rows[maintainPlanLeftTree.resultIdx].vid,resultArr.Rows[maintainPlanLeftTree.resultIdx].vehicleNo);
			}else{
		...
		findVehicleNoHC : function(vehicleArr) {
			...
			maintainPlanLeftTree.resultIdx = 0;
		...
		_searchDataNodeByTreeDataIndex : function(entId,vid){
			var temp = $("ul[id=panelConplan3]:visible").find("li[outlinelevel=1]").find(".l-box.l-checkbox").eq(0);
			if(temp.attr("class").indexOf("l-checkbox-checked")>0){
				temp.click();
			}
			var tmpNode = $("ul[id=panelConplan3]:visible").find("li[id="+entId+"]");
	        ...
	        for (var i = 0; i < data.length; i++)
	        {
	        	if(data[i].childrenList != ""){
		            ...
		            if (data[i].id == entId){//data[i].id
						var tmpNode = $("#panelConplan3").find("li[id="+entId+"]");
	    ...
	    bindPanelVehicleEvent : function(){
			...
			$("#searchplanThree").click(function(){
				obj.clearResult(false);
				var vehicleNo = $("#threeSearchplanInput").val();
				...
				$.ajax({
					   ...
					   success: function(col){
						   maintainPlanLeftTree.searchArr = eval("data="+col);
					   		if(maintainPlanLeftTree.searchArr.Rows.length != 0){
//					   			$("#resultplanOper").removeClass("btnNone");
//					   			$("div[id=panelplan3]:visible").css("padding-top","60");
						   		obj.findVehicleNoHC(col);
						   		//设置查询出的车辆结果填入下拉列表中
						   		var selResultObj = $("select[id=vehicleplanResult]:visible");
					   			selResultObj.removeAttr("disabled");
					   			selResultObj.empty();
					   			selResultObj.data("searchArr",maintainPlanLeftTree.searchArr);
					   			$.each(maintainPlanLeftTree.searchArr.Rows,function(idx,node){
						   			var vid = node.vid;
					                var vehicleNo = node.vehicleNo;

					                $("select[name='vehicleplanResult']",$("#resultplanOper")).append("<option value='"+vid+"'>"+$.trim(vehicleNo)+"</option>");
					                if (idx==0){
	...
	//				                	obj._getDataNodeByTreeDataIndex(null,node);
					   			...
					   			var selResultObj = $("select[name='vehicleplanResult']",$("div[id=accordionplan1]:visible"));
			...
			//车辆查询输入框事件
				$("#threeSearchplanInput").keypress(function(){
					var a = $(this).val();
					if(a == "车牌号"){
						$(this).val("");
...
//				maintainPlanLeftTree.searchArr = [];
//				var heightNode = $("div[id=panelplan3]:visible").find("div[class*=searchBk]");
//				$("#threeSearchplanInput").val("");
//				$(heightNode[0]).removeClass("searchBk");
//				$("#resultplanOper").addClass("btnNone");
//				$("div[id=panelplan3]:visible").css("padding-top","40");
				maintainPlanLeftTree.searchArr = [];
				var heightNode = $("#panelplan3").find("div[class*=searchBk]").removeClass("searchBk");
				if(flag){
					$("#threeSearchplanInput").val("车牌号");
				}
	 			var selResultObj = $("#vehicleplanResult");
	   			selResultObj.attr("disabled",true);
	   			selResultObj.empty();
			},
			bindClearResult : function(){
				var obj = this;
				$("#clearplanResult").click(function(){
					obj.clearResult(true);
				});
			},
			bindSearchDivFloat : function(){
				$("#panelplan3").scroll(function(){
					var offsetTop = $(this).scrollTop();
					$("#panelplan3").find("div[class=panelSearch]").css("top",offsetTop-1+"px");
				});
			},
			//下拉框事件,车辆查询结果
	    resultEvent : function(){
	    	var obj = this;
	    	$("#vehicleplanResult").change(function(){
	    		var vid = $(this).val();
...
function divOffset(liId,vehicleNo){
 	var tmpNode = $("ul[id=panelConplan3]:visible").find("li[id="+liId+"]:contains('"+vehicleNo+"')");
 	$("#panelConplan3").scrollTop(0);
 	$("ul[id=panelConplan3]:visible > li").find("div[class*=l-checkbox-checked]").click();
 	if(tmpNode != null && tmpNode != undefined){
		var arr = $(tmpNode[0]).offset();
		$("#panelConplan3").scrollTop(arr.top-280);
    	var removeClassNode = $("#panelConplan3").find("div[class*=searchBk]");
    	$(removeClassNode[0]).removeClass("searchBk");
		$(tmpNode[tmpNode.length-1]).children().addClass("searchBk");
		$("ul[id=panelConplan3]:visible").find("div[class*=l-checkbox-checked]").click();
    	$(tmpNode[tmpNode.length-1]).find("div[class*=l-checkbox]").click();
	}
};
	maintainPlanLeftTree = new maintainPlanLeftTree();
$(document).ready(function() {
	maintainPlanLeftTree.bindPanelVehicleEvent();
//	maintainPlanLeftTree.bindResultEvent();
	maintainPlanLeftTree.bindClearResult();
	maintainPlanLeftTree.resultEvent();
  });