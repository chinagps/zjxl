var maintainStatisticsLeftTree = function(){
...
maintainStatisticsLeftTree.prototype = {
		init : function(){
			this.load();
		},
		load : function(){
	 		$("#accordionStatistics1").ligerAccordion({ changeHeightOnResize : true,speed : null});
			var obj = this;
			obj.setThree(obj);
		},
		setThree : function(o){
			var treeClick = "treeClick";
			$("#panelConStatistics3").ligerTree({
...
//					o.allSel(true);
			...
			o.treeTreeManager = $("#panelConStatistics3").ligerGetTreeManager();
	 	},
	 	//获取树的高度，以便增加滚动条
	 	bindPanelSize : function(){
	 		var obj = this;
			var panelContextObj = $("#panelConStatistics3");
			//计算高度
			var hgt = 260;
	 	...
	 	onResize:function(){
			$("#panelConStatistics3").height(getHeightAndWidth().height-173);
		},
		...
		treeClick : function(note) {
			try{
				var dix = maintainStatisticsLeftTree.getDisplayPanel();
				var manager = maintainStatisticsLeftTree.getManager(dix);
				var id = note.data.id;
				...
				if(entType == "2"){
					if(dix == "3"){
						if (note.data.childrenList=="" && nodeType == "2"){
							...
							maintainStatisticsLeftTree.ajaxTreeNode(url,note,id,manager,dix);
						}
					}else if(dix == "4"){
						if (note.data.childrenList=="" && nodeType == "2"){
							var tg = note.target;
		...
		ajaxTreeNode : function(url,note,id,manager,idx){
	    	$.ajax({
			   ...
			   success: function(col){
				   maintainStatisticsLeftTree.addTreeNode(note,col,manager);
				   maintainStatisticsLeftTree.selectNode(idx,false);
				   maintainStatisticsLeftTree.selectSubNodeAll(note.target);
			   }
			});
	    },
	    ...
	    selectNode : function (idx,flag)
	    {
	    	var a = null;
	    	if(idx == "3"){
	    		a = $("#panelConStatistics3 > li > div > div[class*=l-checkbox]");
	    	}
		...
		leafCoverFolder : function(){
	    	$("#panelConStatistics3").find("div[class*=l-note]").removeClass("l-note").addClass("l-expandable-close");
	    },
	    bindResultEvent : function(){
			var obj = this;
			$("#resultStatisticsUp").click(function(){
				obj.treeSearchUp();
			});
			$("#resultStatisticsDown").click(function(){
				obj.treeSearchDown();
			});
		},
		//查询结果上移一位
		treeSearchUp : function(idx){
			var obj = this;
			var resultArr = maintainStatisticsLeftTree.searchArr;
			var manager = obj.treeTreeManager;
			data = manager.getData();
			if(maintainStatisticsLeftTree.resultIdx > 0){
				maintainStatisticsLeftTree.resultIdx = Number(idx);
				obj._getDataNodeByTreeDataIndex(data[0].childrenList,resultArr.Rows[maintainStatisticsLeftTree.resultIdx]);
				obj.searchSel(resultArr.Rows[maintainStatisticsLeftTree.resultIdx].vid,resultArr.Rows[maintainStatisticsLeftTree.resultIdx].vehicleNo);
			}else{
		...
		treeSearchDown : function(idx){
			...
			var manager = obj.treeTreeManager;
			data = manager.getData();
			if(maintainStatisticsLeftTree.resultIdx < resultArr.Rows.length-1){
				maintainStatisticsLeftTree.resultIdx = Number(idx);
				obj._getDataNodeByTreeDataIndex(data[0].childrenList,resultArr.Rows[maintainStatisticsLeftTree.resultIdx]);
				obj.searchSel(resultArr.Rows[maintainStatisticsLeftTree.resultIdx].vid,resultArr.Rows[maintainStatisticsLeftTree.resultIdx].vehicleNo);
			}else{
		...
		findVehicleNoHC : function(vehicleArr) {
			...
			maintainStatisticsLeftTree.resultIdx = 0;
		...
		_searchDataNodeByTreeDataIndex : function(entId,vid){
			var temp = $("ul[id=panelConStatistics3]:visible").find("li[outlinelevel=1]").find(".l-box.l-checkbox").eq(0);
			if(temp.attr("class").indexOf("l-checkbox-checked")>0){
				temp.click();
			}
			var tmpNode = $("ul[id=panelConStatistics3]:visible").find("li[id="+entId+"]");
	        ...
	        for (var i = 0; i < data.length; i++)
	        {
	        	if(data[i].childrenList != ""){
		            ...
		            if (data[i].id == entId){//data[i].id
						var tmpNode = $("#panelConStatistics3").find("li[id="+entId+"]");
	    ...
	    bindPanelVehicleEvent : function(){
			...
			$("#searchStatisticsThree").click(function(){
				obj.clearResult(false);
				var vehicleNo = $("#threeSearchStatisticsInput").val();
				...
				$.ajax({
					   ...
					   success: function(col){
						   maintainStatisticsLeftTree.searchArr = eval("data="+col);
					   		if(maintainStatisticsLeftTree.searchArr.Rows.length != 0){
					   			$("#resultStatisticsOper").removeClass("btnNone");
					   			$("div[id=panelStatistics3]:visible").css("padding-top","60");
						   		obj.findVehicleNoHC(col);
						   		//设置查询出的车辆结果填入下拉列表中
						   		var selResultObj = $("#vehicleStatisticsResult");
					   			selResultObj.removeAttr("disabled");
					   			selResultObj.empty();
					   			selResultObj.data("searchArr",maintainStatisticsLeftTree.searchArr);
					   			$.each(maintainStatisticsLeftTree.searchArr.Rows,function(idx,node){
						   			var vid = node.vid;
					                var vehicleNo = node.vehicleNo;

					                $("select[name='vehicleStatisticsResult']",$("#resultStatisticsOper")).append("<option value='"+vid+"'>"+$.trim(vehicleNo)+"</option>");
					                if (idx==0){
	...
	//				                	obj._getDataNodeByTreeDataIndex(null,node);
					   			...
					   			var selResultObj = $("select[name='vehicleStatisticsResult']",$("div[id=accordionStatistics1]:visible"));
			...
			//车辆查询输入框事件
				$("#threeSearchStatisticsInput").keypress(function(){
					var a = $(this).val();
					if(a == "车牌号"){
						$(this).val("");
...
//			var notes = manager.getChecked();
		...
		allSel : function(flag){
			if(flag){
				$("ul[id=panelConStatistics3]:visible > li").find("div[class*=l-checkbox]")[0].click();
			}
		},
				//清除查询车辆查询结果
			clearResult : function(flag){
//				maintainStatisticsLeftTree.searchArr = [];
//				var heightNode = $("div[id=panelStatistics3]:visible").find("div[class*=searchBk]");
//				$("#threeSearchStatisticsInput").val("");
//				$(heightNode[0]).removeClass("searchBk");
//				$("#resultStatisticsOper").addClass("btnNone");
//				$("div[id=panelStatistics3]:visible").css("padding-top","40");
				maintainStatisticsLeftTree.searchArr = [];
				var heightNode = $("#panelStatistics3").find("div[class*=searchBk]").removeClass("searchBk");
				if(flag){	
					$("#threeSearchStatisticsInput").val("车牌号");
				}
	 			var selResultObj = $("#vehicleStatisticsResult");
	   			selResultObj.attr("disabled",true);
	   			selResultObj.empty();
			...
			bindClearResult : function(){
				var obj = this;
				$("#clearStatisticsResult").click(function(){
					obj.clearResult(true);
				});
			},
			bindSearchDivFloat : function(){
				$("#panelStatistics3").scroll(function(){
					var offsetTop = $(this).scrollTop();
					$("#panelStatistics3").find("div[class=panelSearch]").css("top",offsetTop-1+"px");
				});
			},
						//下拉框事件,车辆查询结果
	    resultEvent : function(){
	    	var obj = this;
	    	$("#vehicleStatisticsResult").change(function(){
	    		var vid = $(this).val();
...
function divOffset(liId,vehicleNo){
 	var tmpNode = $("ul[id=panelConStatistics3]:visible").find("li[id="+liId+"]:contains('"+vehicleNo+"')");
 	$("#panelConStatistics3").scrollTop(0);
 	$("ul[id=panelConStatistics3]:visible > li").find("div[class*=l-checkbox-checked]")[0].click();
 	if(tmpNode != null && tmpNode != undefined){
		var arr = $(tmpNode[0]).offset();
		$("#panelConStatistics3").scrollTop(arr.top-280);
    	var removeClassNode = $("#panelConStatistics3").find("div[class*=searchBk]");
    	$(removeClassNode[0]).removeClass("searchBk");
		$(tmpNode[tmpNode.length-1]).children().addClass("searchBk");
		$("ul[id=panelConStatistics3]:visible").find("div[class*=l-checkbox-checked]").click();
    	$(tmpNode[tmpNode.length-1]).find("div[class*=l-checkbox]").click();
	}
};
	maintainStatisticsLeftTree = new maintainStatisticsLeftTree();
$(document).ready(function() {
	maintainStatisticsLeftTree.bindPanelVehicleEvent();
//	maintainStatisticsLeftTree.bindResultEvent();
	maintainStatisticsLeftTree.bindClearResult();
	maintainStatisticsLeftTree.resultEvent();
  });