var maintainDetailLeftTree = function(){
...
maintainDetailLeftTree.prototype = {
		init : function(){
			this.load();
		},
		load : function(){
	 		$("#accordionRecord1").ligerAccordion({changeHeightOnResize : true,speed : null});
			var obj = this;
			obj.setThree(obj);
		},
		onResize:function(){
			$("#panelConRecord3").height(getHeightAndWidth().height-173);
		},
		setThree : function(o){
			var treeClick = "treeClick";
			$("#panelConRecord3").ligerTree({
...
//					o.allSel(true);
			...
			o.treeTreeManager = $("#panelConRecord3").ligerGetTreeManager();
	 	},
	 	//获取树的高度，以便增加滚动条
	 	bindPanelSize : function(){
	 		var obj = this;
			var panelContextObj = $("#panelConRecord3");
			//计算高度
			var hgt = 260;
			panelContextObj.css("overflow","auto").css("width","248px");
		...
		treeClick : function(note) {
			try{
				var dix = maintainDetailLeftTree.getDisplayPanel();
				var manager = maintainDetailLeftTree.getManager(dix);
				var id = note.data.id;
				...
				if(entType == "2"){
					if(dix == "3"){
						if (note.data.childrenList=="" && nodeType == "2"){
							...
							maintainDetailLeftTree.ajaxTreeNode(url,note,id,manager,dix);
						}
					}else if(dix == "4"){
						if (note.data.childrenList=="" && nodeType == "2"){
							var tg = note.target;
		...
		ajaxTreeNode : function(url,note,id,manager,idx){
	    	$.ajax({
			   ...
			   success: function(col){
				   maintainDetailLeftTree.addTreeNode(note,col,manager);
				   maintainDetailLeftTree.selectNode(idx,false);
				   maintainDetailLeftTree.selectSubNodeAll(note.target);
			   }
			});
	    },
	    ...
	    selectNode : function (idx,flag)
	    {
	    	var a = null;
	    	if(idx == "3"){
	    		a = $("#panelConRecord3 > li > div > div[class*=l-checkbox]");
	    	}
		...
		leafCoverFolder : function(){
	    	$("#panelConRecord3").find("div[class*=l-note]").removeClass("l-note").addClass("l-expandable-close");
	    },
		...
		treeSearchUp : function(idx){
			var obj = this;
			var resultArr = maintainDetailLeftTree.searchArr;
			var manager = obj.treeTreeManager;
			data = manager.getData();
			if(maintainDetailLeftTree.resultIdx > 0){
				maintainDetailLeftTree.resultIdx = Number(idx);
				obj._getDataNodeByTreeDataIndex(data[0].childrenList,resultArr.Rows[maintainDetailLeftTree.resultIdx]);
				obj.searchSel(resultArr.Rows[maintainDetailLeftTree.resultIdx].vid,resultArr.Rows[maintainDetailLeftTree.resultIdx].vehicleNo);
			}else{
		...
		treeSearchDown : function(idx){
			...
			var manager = obj.treeTreeManager;
			data = manager.getData();
			if(maintainDetailLeftTree.resultIdx < resultArr.Rows.length-1){
				maintainDetailLeftTree.resultIdx = Number(idx);
				obj._getDataNodeByTreeDataIndex(data[0].childrenList,resultArr.Rows[maintainDetailLeftTree.resultIdx]);
				obj.searchSel(resultArr.Rows[maintainDetailLeftTree.resultIdx].vid,resultArr.Rows[maintainDetailLeftTree.resultIdx].vehicleNo);
			}else{
		...
		findVehicleNoHC : function(vehicleArr) {
			...
			maintainDetailLeftTree.resultIdx = 0;
		...
		_searchDataNodeByTreeDataIndex : function(entId,vid){
			var temp = $("ul[id=panelConRecord3]:visible").find("li[outlinelevel=1]").find(".l-box.l-checkbox").eq(0);
			if(temp.attr("class").indexOf("l-checkbox-checked")>0){
				temp.click();
			}
			var tmpNode = $("ul[id=panelConRecord3]:visible").find("li[id="+entId+"]");
	        ...
	        for (var i = 0; i < data.length; i++)
	        {
	        	if(data[i].childrenList != ""){
		            ...
		            if (data[i].id == entId){//data[i].id
						var tmpNode = $("#panelConRecord3").find("li[id="+entId+"]");
	    ...
	    bindPanelVehicleEvent : function(){
			...
			$("#searchReportThree").click(function(){
				obj.clearResult(false);
				var vehicleNo = $("#threeSearchRecordInput").val();
				...
				$.ajax({
					   ...
					   success: function(col){
						   maintainDetailLeftTree.searchArr = eval("data="+col);
					   		if(maintainDetailLeftTree.searchArr.Rows.length != 0){
//					   			$("#resultRecordOper").removeClass("btnNone");
//					   			$("div[id=panelRecord3]:visible").css("padding-top","60");
						   		obj.findVehicleNoHC(col);
						   		//设置查询出的车辆结果填入下拉列表中
						   		var selResultObj = $("select[id=vehicleRecordResult]:visible");
					   			selResultObj.removeAttr("disabled");
					   			selResultObj.empty();
					   			selResultObj.data("searchArr",maintainDetailLeftTree.searchArr);
					   			$.each(maintainDetailLeftTree.searchArr.Rows,function(idx,node){
						   			var vid = node.vid;
					                var vehicleNo = node.vehicleNo;

					                $("select[name='vehicleRecordResult']",$("#resultRecordOper:visible")).append("<option value='"+vid+"'>"+$.trim(vehicleNo)+"</option>");
					                if (idx==0){
	...
	//				                	obj._getDataNodeByTreeDataIndex(null,node);
					   			...
					   			var selResultObj = $("select[name='vehicleRecordResult']",$("div[id=accordionRecord1]:visible"));
			...
			//车辆查询输入框事件
				$("#threeSearchRecordInput").keypress(function(){
					var a = $(this).val();
					if(a == "车牌号"){
						$(this).val("");
...
//			var notes = manager.getChecked();
		...
		allSel : function(flag){
			if(flag){
				$("ul[id=panelConRecord3]:visible > li").find("div[class*=l-checkbox]")[0].click();
			}
		},
				//清除查询车辆查询结果
		clearResult : function(flag){
//				maintainDetailLeftTree.searchArr = [];
...
//				$("div[id=panelRecord3]:visible").css("padding-top","40");
				maintainDetailLeftTree.searchArr = [];
			var heightNode = $("#panelRecord3").find("div[class*=searchBk]").removeClass("searchBk");
			if(flag){
				$("#threeSearchRecordInput").val("车牌号");
			}
 			var selResultObj = $("#vehicleRecordResult");
   			selResultObj.attr("disabled",true);
   			selResultObj.empty();
		},
		bindClearResult : function(){
			var obj = this;
			$("#clearRecordResult").click(function(){
				obj.clearResult(true);
			});
		},
		bindSearchDivFloat : function(){
			$("#panelRecord3").scroll(function(){
				var offsetTop = $(this).scrollTop();
				$("#panelRecord3").find("div[class=panelSearch]").css("top",offsetTop-1+"px");
			});
		},
			//下拉框事件,车辆查询结果
	    resultEvent : function(){
	    	var obj = this;
	    	$("#vehicleRecordResult").change(function(){
	    		var vid = $(this).val();
...
function divOffset(liId,vehicleNo){
	var tmpNode = $("ul[id=panelConRecord3]:visible").find("li[id="+liId+"]:contains('"+vehicleNo+"')");
 	$("#panelConRecord3").scrollTop(0);
 	$("ul[id=panelConRecord3]:visible > li").find("div[class*=l-checkbox-checked]")[0].click();
 	if(tmpNode != null && tmpNode != undefined){
		var arr = $(tmpNode[0]).offset();
		$("#panelConRecord3").scrollTop(arr.top-280);
    	var removeClassNode = $("#panelConRecord3").find("div[class*=searchBk]");
    	$(removeClassNode[0]).removeClass("searchBk");
		$(tmpNode[tmpNode.length-1]).children().addClass("searchBk");
		$("ul[id=panelConRecord3]:visible").find("div[class*=l-checkbox-checked]").click();
    	$(tmpNode[tmpNode.length-1]).find("div[class*=l-checkbox]").click();
	}
};
	maintainDetailLeftTree = new maintainDetailLeftTree();
$(document).ready(function() {
	maintainDetailLeftTree.bindPanelVehicleEvent();
	//maintainDetailLeftTree.bindResultEvent();
	maintainDetailLeftTree.bindClearResult();
	maintainDetailLeftTree.resultEvent();
