...
{
    var l = $.ligerui;
     //modify by tangfeng 2012-09-14 翻页之后排序  不生效问题 （加一个变量缓存前一页面值）
    var pageCachedNow=1;
    ...
    $.ligerDefaults.Grid = {
		...
		DataBase : "sub-total" // 数据库中每列合计使用的字段
    };
...
//            gridhtmlarr.push("                </div>");
//            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'><span class='l-bar-text'></span></div>");
        ...
        {
            var g = this, p = this.options, ch = h;
            ...
            {
                g.gridbody.height(h);
                if (h > 18) g.f.gridbody.height(h - 18 + 1);
                g.gridview.height(h + gridHeaderHeight);
            }
            p.tableBody = h;
            p.height = ch;//add by fanshine 
        },
        _updateFrozenWidth: function ()
        {
            ...
            if (g.enabledFrozen())
            {
                ...
                var view2width = g.gridtablewidth;//g.gridview.width() - g.f.gridtablewidth;
        ...
        loadDataOut : function(loadService, newUrl) {
			var g = this, p = this.options;
			p.newPage = 1;

			this.loadData(loadService, newUrl);
		},
		initPager : function() {
			var g = this, p = this.options;
			if(p.barTextShow){
				var stat = p.pageStatMessage;
				stat = stat.replace(/{from}/, 0);
				stat = stat.replace(/{to}/, 0);
				stat = stat.replace(/{total}/, 0);
		...
		reloadColumn : function(cloumns) {
			var g = this, p = this.options;
			g.headers = [];
			$(".l-grid-hd-cell", grid).remove();
			p.columns = cloumns;
			// 重新加载列，g.gridtablewidth重置，避免标题列不随内容滚动
			g.gridtablewidth = 0;
			$(cloumns).each(function(i, item) {
				var $headerCell = $("<td class='l-grid-hd-cell' columnindex='"
						+ i
						+ "'><div class='l-grid-hd-cell-inner'><span class='l-grid-hd-cell-text'> </span></div></td>");
		...
		//////////////////
        //刷新数据
        loadData: function (loadDataParm, newUrl)
        {
            var g = this, p = this.options;
            //add by fanshine
//            if (p.checkbox) {
//				var aa = $(g.grid).find(".l-grid-header-table").find("tr").first().removeClass("l-checked");
//			}
            //
        ...
        loadServerData: function (param, clause)
        {
            ...
            var ajaxOptions = {
                type: p.method,
                url: p.url,
            	timeout : p.timeout ? p.timeout : 20000,
                data: param,
                async: p.async,
                dataType: 'json',
                beforeSend: function ()
        ...
        */
        ...
        changeSort: function (columnName, sortOrder)
        {
          ...
          //modify by tangfeng 2012-09-06 前台排序支持url方式  
           // if (p.dataAction == "local")
            {
              ...
              //modify by tangfeng 2012-09-06 前台排序支持url方式  
                ...
                if (!g.sortedData)
                 ...
                 //modify by tangfeng 2012-09-14 翻页之后排序  不生效问题 （加一个变量缓存前一页面值）   
                if (p.sortName == columnName && pageCachedNow==p.page)
                {
                ...
                } else
                {
                    g.sortedData[p.root].sort(function (data1, data2)
                    {
                     ...
                     //modify by tangfeng 2012-09-14 翻页之后排序  不生效问题 （加一个变量缓存前一页面值）
                    pageCachedNow=p.page;
                }
                 //modify by tangfeng 2012-09-14 翻页之后排序页面Grid内容不能加载问题 _getCurrentPageData方法有问题
//                if (p.usePager)
...
//                    g.currentData = g.sortedData;
            ...
            //modify by tangfeng 2012-09-06 前台排序支持url方式 start 
//            if (p.dataAction == "server")
//            {
//                g.loadData(p.where);
//            }
            //modify by tangfeng 2012-09-06 前台排序支持url方式 end 
        },
        //改变分页
        changePage: function (ctype)
        {
            ...
            {
                //add by fanshine 
            	var hasInPageActionParam = false;
				$(p.parms).each(function() {
					if (this.name == "queryParam.isInPageAction")
						hasInPageActionParam = true;
				});
				if (p.inPageAction && !hasInPageActionParam) {
					p.parms.push( {
						name : "queryParam.isInPageAction",
						value : true
					});
				}
				///////////////////////
        ...
        _getHtmlFromData: function (data, frozen)
        {
            ...
            for (var rowparm in data)
            {
                ...
                // add by fanshine 添加checkbox状态记忆
                ...
                // add by fanshine 增加保存的id
				if (p.selectId) {
					gridhtmlarr.push('selectid = ' + item[p.selectId]);
				}
        ...
        _onClick: function (e)
        {
            ...
            if (src.checkboxall) //复选框全选
            {
                ...
                for (var rowid in g.records)
                {
                	//add by tangfeng 20121027 排序后复选框样式问题 start
                   
                    var rowdata = g.getRow(src.data);
                    var rowobj1 = g.getRowObj(rowid);
                    var rowobj2 = g.getRowObj(rowid, true);
                    var uncheck1 = $(rowobj1).hasClass("l-checked"); 
                    if (uncheck1)
                    {
                    	$(rowobj1).removeClass("l-checked");
                    	$(rowobj2).removeClass("l-checked");
                    }
//                    else
//                    {
//                    	$(rowobj1).addClass("l-checked");
//                    	$(rowobj2).addClass("l-checked");
//                    }
                    //add by tangfeng 20121027 排序后复选框样式问题 end
                    if (uncheck)
                        g.unselect(g.records[rowid]);
                    else
                        g.select(g.records[rowid]);
                }
                g.trigger('checkAllRow', [!uncheck, g.element, g.currentData]);
            }
            ...
            else if (src.row && g.enabledCheckbox()) //复选框选择行
            {
                ...
                if (src.checkbox || !selectRowButtonOnly)
                {
                    ...
                    if (g.trigger('beforeCheckRow', [!uncheck, src.data, src.data['__id'], src.row, obj]) == false)
                        return false;
                    //add by tangfeng 20121027 排序后复选框样式问题 start
                    var uncheck1 = row.hasClass("l-checked"); 
                    var rowdata = g.getRow(src.data);
                    var rowid = rowdata['__id'];
                    var rowobj1 = g.getRowObj(rowid);
                    var rowobj2 = g.getRowObj(rowid, true);
...
//                    }
                    ...
                    g.trigger('checkRow', [!uncheck, src.data, src.data['__id'], src.row, obj]);
                }
        ...
        select: function (rowParm, obj)
        {
            ...
            g.trigger('selectRow', [rowdata, rowid, rowobj, obj]);
        },
        ...
        _onResize: function ()
        {
            ...
            if (g.enabledFrozen())
            {
                var gridView1Width = (g.gridview1.width() > 0 ? g.gridview1.width() : 30);
                var gridViewWidth = (g.gridview.width() > 0 ? g.gridview.width() : 270);
                //begin modify jiangwei 修改grid在隐藏的时候onresize宽度计算有问题
                if(g.gridview.width()==0){
                	g.gridview2.css({
	                    width: '100%'
	                });
                }else{
              ...
              //end modify jiangwei 修改grid在隐藏的时候onresize宽度计算有问题
            }
            g.trigger('SysGridHeightChanged');
        }
    });
