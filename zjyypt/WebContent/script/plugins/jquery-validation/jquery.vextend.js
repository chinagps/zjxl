...
phonenumber:function(value,element){
	...
	if(r==null) return this.optional(element) ||false; 
...
$(document).ready(function() {
	...
	// 字符验证
	jQuery.validator.addMethod("specialchars", validatekcpt.specialchars, "不能含特殊符号");
	// 电话号码
	jQuery.validator.addMethod("phonenumber", validatekcpt.phonenumber, "无效电话号码");
	// 电话邮编
	jQuery.validator.addMethod("zipcode", validatekcpt.zipcode, "无效邮编");
	// 手机号码
	jQuery.validator.addMethod("mobile", validatekcpt.mobile, "无效手机号");
	// 身份证号
	jQuery.validator.addMethod("isidcardno", validatekcpt.isidcardno, "无效身份证号");
	//电子邮件地址
	jQuery.validator.addMethod("emailExtend", validatekcpt.emailExtend, "无效邮箱");
	
	// 身份证号
	jQuery.validator.addMethod("mobilePhoneNum", validatekcpt.mobilePhoneNum, "无效联系电话");
    // 验证值小数位数不能超过两位 
	jQuery.validator.addMethod("decimal", function(value, element) {
	 	var decimal = /^-?\d+(\.\d{1,2})?$/;
 		return this.optional(element) || (decimal.test(value));},"小数位数不能超过两位");
	//验证值为非负数 
	jQuery.validator.addMethod("plusNumber", function(value, element) {
	 return this.optional(element) || (value>=0);},"不能为负数");
	// 验证汉字
	jQuery.validator.addMethod("noChinese", function(value, element) {
		var noChinese = /^[^\u4E00-\u9FA5\uF900-\uFA2D]+$/;///^-?\d+(\.\d{1,2})?$/;///[\u4E00-\u9FA5\uF900-\uFA2D]/
		return this.optional(element) || (noChinese.test(value));},"不能包含汉字");
	// 验证非负整数
	jQuery.validator.addMethod("intNumber", function(value, element) {
		var intNumber = /^\d+$/;///^-?\d+(\.\d{1,2})?$/;///[\u4E00-\u9FA5\uF900-\uFA2D]/
		return this.optional(element) || (intNumber.test(value));},"请输入正确数字");
	// 验证字母或者数字组合
