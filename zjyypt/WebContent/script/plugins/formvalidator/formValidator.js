...
(function($) {
	...
	$.formValidator = {
		...
		initConfig : function(controlOptions) {
			...
			if (settings.formid != "") {
				$("#" + settings.formid).submit(function() {
					return $.formValidator.pageIsValid(settings.formid);
				})
			}
			;
			...
			for ( var i = 0; i < jQuery_formValidator_initConfig.length; i++) {
				if (jQuery_formValidator_initConfig[i].formid == settings.formid) {
					jQuery_formValidator_initConfig.splice(i, 1);
		...
		getInitConfig : function(validatorgroup) {
			if (jQuery_formValidator_initConfig != null) {
				for (i = 0; i < jQuery_formValidator_initConfig.length; i++) {
					if (jQuery_formValidator_initConfig[i].formid == validatorgroup) {
		...
		setTipState : function(elem, showclass, showmsg) {
			try {
				...
				var initConfig = $.formValidator.getInitConfig(setting0.formid);
		...
		resetAllTipState : function(validatorgroup) {
			var initConfig = $.formValidator
					.getInitConfig(validatorgroup.formid);
			$(initConfig.validobjectids).each(
					function() {
						$.formValidator.setTipState(this, "onShow",
								this.settings[0].onshow);
						$.formValidator.resetAllValidator(this, false);
					});

		},
		// ajax 请求后再重置后要把控件的属性改为开始状态这样才能继续判断
		resetAllValidator : function(elem, t) {
			if (!elem && !elem.settings)
				return;
		...
		showMessage : function(returnObj) {
			...
			var intiConfig = $.formValidator.getInitConfig(settings[0].formid);
			if (!isvalid) {
				showclass = "onError";
		...
		showAjaxMessage : function(returnObj) {
			...
			$.formValidator.ajaxValid(returnObj);
			/*
			 * } else { if(setting.isvalid!=undefined && !setting.isvalid){
			 * elem.lastshowclass = "onError"; elem.lastshowmsg =
			 * setting.onerror; }
		...
		getLength : function(id) {
			...
			case "file":
				...
				var initConfig = $.formValidator
						.getInitConfig(elem.settings[0].formid);
		...
		oneIsValid : function(id, index) {
			...
			if(dis){
				...
				returnObj.setting = {
						...
						validatetype : "FunctionsValidator",
						oncorrect : ""
					};
			...
			if (filtrates($(elem).attr("name"), $(elem).attr("value").trim(),true) == false) {
		...
		pageIsValid : function(validatorgroup) {
			...
			var initConfig = initConfig = $.formValidator
					.getInitConfig(validatorgroup);

			var jqObjs = $(initConfig.validobjectids);

			jqObjs
					.each(function(i, elem) { // alert($(elem).attr("disabled"))
						if (!$(elem).attr("disabled")) {
							if (elem.settings[0].bind) {

								returnObj = $.formValidator.oneIsValid(elem.id,
										1);
		...
		ajaxValid : function(returnObj) {
			...
			if (setting.addidvalue) {
				...
				ls_url = ls_url
						+ (ls_url.indexOf("?") > 0 ? ("&" + parm)
								: ("?" + parm));
			}
			if (setting.data) {
				var attr = setting.addParam.split("!");
				    var validated;
					for ( var i = 0; i < attr.length; i++) {
						var tempVal = false;
						setting.data[attr[i]] = setting.addValue[i].call(this);
						// 添加
						if(i==0){
							if (setting.defaultValue&& setting.defaultValue[i] == attr[i]) {
								var dv = setting.defaultValue[i+1].call(this);
								if (dv && dv == setting.addValue[i].call(this)) {
									tempVal = true;
								}else{
									tempVal = false;
								}
							...
							if (setting.defaultValue&& setting.defaultValue[i+1] == attr[i]) {
								var dv = setting.defaultValue[i+2].call(this);
								if (dv && dv == setting.addValue[i].call(this)) {
									tempVal = true;
								}else{
									tempVal = false;
						...
						if(validated==undefined){
							...
							if(validated==true&&tempVal==true){
								validated = true;
							}else{
								validated = false;
							}
				...
				var attr = setting.addParam.split("!");
				if (attr.length == 1) {
					setting.data[setting.addParam] = srcjo.attr("name") + "="
							+ srcjo.val();
					if (setting.defaultValue) {
						var dv = setting.defaultValue[1].call(this);
						if (dv && dv == srcjo.val()) {
							$.formValidator.setTipState(elem, "onCorrect",
									settings[0].oncorrect);
							setting.isvalid = true;
					...
					for ( var i = 1; i < attr.length; i++) {
						if (i > 1)
							setting.data[attr[0]] = setting.data[attr[0]] + "&"
									+ attr[i] + "="
									+ setting.addValue[i - 1].call(this);
						else
							setting.data[attr[0]] = attr[i] + "="
									+ setting.addValue[i - 1].call(this);
						// 添加
						if (setting.defaultValue
								&& setting.defaultValue[0] == attr[i]) {
							var dv = setting.defaultValue[1].call(this);
							if (dv && dv == setting.addValue[i - 1].call(this)) {
								$.formValidator.setTipState(elem, "onCorrect",
										settings[0].oncorrect);
								setting.isvalid = true;
								return;
					...
					// setting.data[attr[0]] =
					// attr[1]+"="+setting.addValue.call(this);
				}
			}
		...
		regexValid : function(returnObj) {
			...
			if (elem.settings[0].empty && elem.value == "") {
				...
				var regs = regexpress.split(",");
				if (regs.length > 0) {
					for ( var m = 0; m < regs.length; m++) {
						var regsO = regs[m];
						if (setting.datatype == "enum") {
							regsO = eval("regexEnum." + regsO);
						}
						if (regsO == undefined || regsO == "") {
							setting.isvalid = false;
							return;
						}
						setting.isvalid = (new RegExp(regsO, setting.param))
								.test($("#" + id).val());
						if (!setting.isvalid) {
							break;
						}
	...
	$.fn.formValidator = function(cs) {
		...
		;
		var initConfig = $.formValidator.getInitConfig(cs.formid);
				...
				.each(function(e) {
					...
					if (initConfig.autotip) {
						...
						if ($("body [id=" + tip + "]").length == 0) {
							...
							$("<div class='formValidateTip'></div>").appendTo(
									$("body")).css({
								left : x + "px",
								top : y + "px"
							}).prepend(
					...
					if (srcTag == "input" || srcTag == "textarea") {
						...
						jqobj.focus(function() {
							...
							if (stype == "password" || stype == "text"
									|| stype == "textarea" || stype == "file") {
		...
		return this
				.each(function() {
					...
					for ( var i = 1; i < settings.length; i++) {
						...
						if (!$.formValidator.getInitConfig(settings[0].formid).alertmessage) {
